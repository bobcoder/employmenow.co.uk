<?php
  ...
  function limit_text($text, $limit) {
    ...
    function get_distance($origin, $destination){
        $url = "http://maps.googleapis.com/maps/api/distancematrix/json?origins=$origin&destinations=$destination&mode=driving&language=en&units=imperial&sensor=false";
        $data = file_get_contents($url);
//var_dump(json_decode($data, true));

        //$data = utf8_decode($data);
        $obj = json_decode($data, true);
        //return ($obj->rows[0]->elements[0]->distance->text); //mile*/
        return $obj;
    }
?>